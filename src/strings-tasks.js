...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const indremove = str.lastIndexOf(value);
    return str.slice(0, indremove) + str.slice(indremove + value.length);
  }
  return str;
}
...
function countVowels(str) {
  if (str.length === 0 || str.match(/[a,e,i,o,u,y,A,E,I,O,U,Y]/gi) === null) {
    return 0;
  }
  return str.match(/[a,e,i,o,u,y,A,E,I,O,U,Y]/gi).length;
}
...
function isPalindrome(str) {
  const strNew = str.toLowerCase().replace(/\W+/g, '');
...
function extractNameFromTemplate(value) {
  const newStr = value.match(/, ([\w\s]+)!/i);
  return newStr[1];
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[\w]/gi, (letter) => cipher[alphabete.indexOf(letter)]);
}
